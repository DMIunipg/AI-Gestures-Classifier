...
double RBFNetwork::startTraining(int num_rbf_units,
                                 double learning_rate,
                                 int num_iterations,
                                 double &mse,
                                 bool calc_accuracy_mse,
                                 bool print_flag)
{
	if(print_flag)
	{
	...
	{	
        ...
        if(print_flag || calc_accuracy_mse)
		{
			...
			printf("Training (%*d/%d), MSE=[%.3f], Acc=[%.2f], Progress [%.2f]\r",
                   ...
                   mse,
                   accuracy*100.0 ,
                   ((double)((double)(iter+1)/(double)num_iterations) * 100.0));
		}
