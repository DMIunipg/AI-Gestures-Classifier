#ifndef KISSFFT_CLASS_H
#define KISSFFT_CLASS_H
#pragma once
#include <complex>
#include <vector>
...
class KissFF
{
    public:
        ...
        KissFF(int nfft,bool inverse,const traits_type & traits=traits_type() ) 
        ...
        void transform(const std::vector < cpx_type >& src, std::vector < cpx_type >& dst)
        {
            kf_work(0, &dst[0], &src[0], 1,1);
        }
		
        void normalize(std::vector < cpx_type >& srcdst) const
        {
			for(auto& value:srcdst) value /= (T_Scalar)_nfft;
        }
